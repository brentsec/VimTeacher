...
assert_test(delete_multiple_lines.type == "insert", "type must be 'insert', got " .. tostring(delete_multiple_lines.type))
...
assert_test(#delete_multiple_lines.allowed_keys == 0, "allowed_keys must be empty, got " .. #delete_multiple_lines.allowed_keys)
...
assert_test(
  ...
  "expected_lines must have fewer lines than snippet_lines, got " .. #challenge.expected_lines .. " >= " .. #challenge.snippet_lines
)
...
assert_test(#challenge.expected_lines < #challenge.snippet_lines or lines_differ, "snippet_lines and expected_lines must differ")
...
for idx, c in ipairs(challenges) do
  ...
  assert_test(
    ...
    "Challenge " .. idx .. ": expected_lines count " .. #c.expected_lines
      .. " must equal snippet_lines count - " .. lines_removed .. " (" .. (#c.snippet_lines - lines_removed) .. ")"
  )
  ...
  elseif c.key == "dk" then
    ...
    assert_test(
      target_row > 1,
      "Challenge " .. idx .. ": dk requires row above target, but target is first row"
    )
  ...
  elseif c.key == "d2j" then
    ...
    assert_test(
      target_row + 2 <= #edited,
      "Challenge " .. idx .. ": d2j requires 2 rows below target"
    )
  ...
  elseif c.key == "d2k" then
    ...
    assert_test(
      target_row > 2,
      "Challenge " .. idx .. ": d2k requires 2 rows above target"
    )
    table.remove(edited, target_row)     -- Remove target row first
  ...
  for i = 1, #c.expected_lines do
    assert_test(
      ...
      "Challenge " .. idx .. " line " .. i .. ": got '" .. tostring(edited[i])
        .. "' expected '" .. tostring(c.expected_lines[i]) .. "'"
    )
  end
end
