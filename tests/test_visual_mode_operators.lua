...
assert_test(visual_mode_operators.type == "insert", "type must be 'insert', got " .. tostring(visual_mode_operators.type))
...
assert_test(#visual_mode_operators.allowed_keys == 1, "allowed_keys must have 1 entry, got " .. #visual_mode_operators.allowed_keys)
...
assert_test(
  challenge.select_end.col >= challenge.target.col,
  "select_end.col must be >= target.col"
)
...
for idx, c in ipairs(challenges) do
  ...
  assert_test(
    ...
    "Challenge " .. idx .. ": select_end.col " .. c.select_end.col
      .. " must be >= target.col " .. c.target.col
  )
  assert_test(
