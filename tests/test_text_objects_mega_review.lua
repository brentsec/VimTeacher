...
assert_test(text_objects_mega_review.type == "insert", "type must be 'insert', got " .. tostring(text_objects_mega_review.type))
...
assert_test(#text_objects_mega_review.allowed_keys == 1, "allowed_keys must have 1 entry, got " .. #text_objects_mega_review.allowed_keys)
...
for idx, c in ipairs(challenges) do
  ...
  if key:match("^di[%(%[{]") or key:match("^da[%(%[{]") or key:match("^ci[%(%[{]") or key:match("^ca[%(%[{]") then
    ...
    if key:match("%[") then open_char, close_char = "[", "]"
    elseif key:match("{") then open_char, close_char = "{", "}" end
    ...
    if open_pos and close_pos then
      ...
      if key:sub(1, 1) == "c" then
        ...
        if key:sub(1, 2) == "ca" then insert_pos = open_pos end
  ...
  elseif key:match("^di[\"']") or key:match("^da[\"']") or key:match("^ci[\"']") or key:match("^ca[\"']") then
    ...
    if first_quote and second_quote then
      ...
      if key:sub(1, 1) == "c" then
        ...
        if key:sub(1, 2) == "ca" then insert_pos = first_quote end
        edited[row + 1] = edited[row + 1]:sub(1, insert_pos) .. c.char .. edited[row + 1]:sub(insert_pos + 1)
      end
    end
  ...
  assert_test(
    ...
    "Challenge " .. idx .. " (" .. c.key .. "): edited line count " .. #edited
      .. " != expected " .. #c.expected_lines
  )
  for i = 1, #c.expected_lines do
    assert_test(
      edited[i] == c.expected_lines[i],
      "Challenge " .. idx .. " (" .. c.key .. ") line " .. i .. ": got '" .. (edited[i] or "nil")
