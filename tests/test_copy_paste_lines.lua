...
assert_test(
	...
	"expected_lines must have 1 more line than snippet_lines (paste adds a line), got "
		...
		.. " expected "
		.. (#challenge.snippet_lines + 1)
)

-- Test 6: Target is the yank_row at column 0
assert_test(challenge.target.row == challenge.yank_row, "target.row must equal yank_row")
assert_test(challenge.target.col == 0, "target.col must be 0, got " .. challenge.target.col)
...
for idx, c in ipairs(challenges) do
	...
	assert_test(
		...
		"Challenge "
			...
			.. #c.expected_lines
			.. " must equal snippet_lines count + 1 ("
			.. (#c.snippet_lines + 1)
			.. ")"
	)
	...
	assert_test(c.target.row == c.yank_row, "Challenge " .. idx .. ": target.row must equal yank_row")
	assert_test(c.target.col == 0, "Challenge " .. idx .. ": target.col must be 0, got " .. c.target.col)
...
counter.finish("test_copy_paste_lines")
