...
assert_test(#delete_lines.allowed_modify_keys == 3, "allowed_modify_keys must have 3 entries, got " .. #delete_lines.allowed_modify_keys)
local modify_set = {}
...
if challenge.key == "dd" then
  assert_test(
    ...
    "dd: expected_lines must have 1 fewer line"
  )
elseif challenge.key == "D" then
  assert_test(
    #challenge.expected_lines == #challenge.snippet_lines,
    "D: expected_lines must have same line count"
  )
end
...
for idx, c in ipairs(challenges) do
  ...
  if c.key == "dd" then
    ...
    -- Verify line count
    assert_test(
      #edited == #c.snippet_lines - 1,
      "Challenge " .. idx .. ": dd should remove 1 line"
    )
    assert_test(
  ...
  elseif c.key == "D" then
    ...
    assert_test(
      #edited == #c.snippet_lines,
      "Challenge " .. idx .. ": D should keep same line count"
    )
    assert_test(
