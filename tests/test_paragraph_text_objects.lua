...
assert_test(paragraph_text_objects.type == "insert", "type must be 'insert', got " .. tostring(paragraph_text_objects.type))
...
assert_test(#paragraph_text_objects.allowed_keys == 1, "allowed_keys must have 1 entry, got " .. #paragraph_text_objects.allowed_keys)
...
for idx, c in ipairs(challenges) do
  if c.key == "dip" then
    ...
    assert_test(
      ...
      "Challenge " .. idx .. " (dip): expected_lines (" .. #c.expected_lines
        .. ") must be < snippet_lines (" .. #c.snippet_lines .. ")"
    )
  elseif c.key == "dap" then
    dap_found = true
    assert_test(
      #c.expected_lines < #c.snippet_lines,
      "Challenge " .. idx .. " (dap): expected_lines (" .. #c.expected_lines
        .. ") must be < snippet_lines (" .. #c.snippet_lines .. ")"
    )
  end
end
