...
assert_test(
  ...
  "Start and target should be on same line: start row " .. challenge.start_pos.row
    .. " != target row " .. challenge.target.row
)

-- Test 7: Start and target are at different columns
assert_test(
  challenge.start_pos.col ~= challenge.target.col,
  "Start and target must have different columns"
)
...
for i = 1, 50 do
  ...
  assert_test(
    ...
    "Generation " .. i .. ": start row " .. ch.start_pos.row
      .. " != target row " .. ch.target.row
  )
  ...
  assert_test(
    ch.start_pos.col ~= ch.target.col,
    "Generation " .. i .. ": start col == target col"
  )
end
