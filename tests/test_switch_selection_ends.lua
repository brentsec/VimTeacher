...
assert_test(desc_text:find("[Vv]isual") ~= nil or desc_text:find("visual mode") ~= nil, "Description should mention visual mode")
assert_test(desc_text:find(" o ") ~= nil or desc_text:find("'o'") ~= nil or desc_text:find('"o"') ~= nil or desc_text:find("Press o") ~= nil, "Description should mention 'o' key")
assert_test(desc_text:find("[Ss]election") ~= nil or desc_text:find("end") ~= nil, "Description should mention selection or ends")

-- Test 7: Hint lines mention visual mode and o key
local hint_text = table.concat(switch_selection_ends.hint_lines, " ")
assert_test(hint_text:find("v") ~= nil or hint_text:find("visual") ~= nil or hint_text:find("Visual") ~= nil, "Hints should mention visual mode")
...
assert_test(challenge.snippet_lines ~= switch_selection_ends.sandbox_snippet, "snippet_lines should not be same reference as sandbox_snippet")
...
-- Summary
print(string.format("test_switch_selection_ends: %d passed, %d failed (total: %d assertions)",
